...
class CMB2_Field {
	...
	public function render_field() {
		...
		esc_attr( $this->peform_param_callback( 'before_row' ) );

		printf( "<div class=\"cmb-row %s\">\n", esc_attr( $this->row_classes() ) );
		...
		if ( 'title' == $this->type() || ! $this->args( 'show_names' ) ) {
			...
			if ( ! $this->args( 'show_names' ) ) {
				...
				printf( "\n<label%s for=\"%s\">%s</label>\n", esc_html( $style ), esc_html( $this->id() ), esc_html( $this->args( 'name' ) ) );
			}
		} else {

			if ( $this->args( 'name' ) ) {
                               $id_this   = $this->id();
                               $args_this = $this->args( 'name' );
				printf( '<div class="cmb-th"><label for="%1$s">%2$s</label></div>', esc_attr( $id_this ), wp_kses_post( $args_this ) );
			}
	...
	public function row_classes() {
		...
		$conditional_classes = array(
			...
			'table-layout'           => in_array( $this->type(), $repeat_table_rows_types, true ),
		);
		...
		foreach ( $conditional_classes as $class => $condition ) {
			if ( $condition ) {
                ...
                $added_classes = $this->get_param_callback_result( 'row_classes', false );
		if ( $added_classes ) {
	...
	public function get_param_callback_result( $param, $echo = true ) {
		...
		if ( array_key_exists( $param, $this->callback_results ) ) {
			// send it back
			return esc_html( $this->callback_results[ $param ] );
		}
		...
		if ( $cb ) {
			if ( $echo ) {
				...
				ob_start();
                                $functino_result = call_user_func( $cb, $this->args(), $this );
				echo esc_attr( $functino_result );
	...
	public function _set_field_defaults( $args ) {
		...
		$args['options'] = 'group' == $args['type'] ? wp_parse_args(
            $args['options'],
            array(
