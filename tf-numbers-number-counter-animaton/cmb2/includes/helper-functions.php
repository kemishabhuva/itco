...
function cmb2_print_metabox_form( $meta_box, $object_id = 0, $args = array() ) {
	...
	$args = wp_parse_args(
        $args,
        array(
			'form_format' => '<form class="cmb-form" method="post" id="%1$s" class="post_edit_number" enctype="multipart/form-data" encoding="multipart/form-data"><input type="hidden" name="object_id" value="%2$s">%3$s<input type="submit" name="submit-cmb" value="%4$s" class="button-primary"></form>',
	...
	if ( $cmb->prop( 'save_fields' )
		// check nonce
		&& wp_unslash( isset( $_POST['submit-cmb'], $_POST['object_id'], $_POST[ $cmb->nonce() ] ) )
		&& wp_verify_nonce( $retrieved_nonce, 'tf_contr_license-options' )
		&& $object_id && $_sanitize_text_field( wp_unslash( isset( $_POST['object_id'] ) ) ) == $object_id
	) {
	...
	// Show cmb form
	printf( esc_attr( $format_parts[0] ), esc_attr( $cmb->cmb_id ), esc_attr( $object_id ) );
	$cmb->show_form();
