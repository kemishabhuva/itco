	...
	protected function repeat_row( $disable_remover = false, $class = 'cmb-repeat-row' ) {
		...
		<div class="cmb-row <?php echo esc_attr( $class ); ?>">
			...
			<div class="cmb-td cmb-remove-row">
				<button class="button cmb-remove-row-button<?php echo esc_html( $disabled ); ?>"><?php echo esc_html( $this->_text( 'remove_row_text', esc_html__( 'Remove', 'cmb2' ) ) ); ?></button>
			</div>
		</div>
...
//	}
	...
	public function text_money() {
		return ( ! $this->field->get_param_callback_result( 'before_field' ) ? '$ ' : ' ' ) . $this->input(
            array(
	...
	public function taxonomy_multicheck() {
		...
		if ( ! $terms ) {
			...
			foreach ( $terms as $term ) {
				...
				if ( is_array( $saved_terms ) && in_array( $term->slug, $saved_terms, true ) ) {
					$args['checked'] = 'checked';
				}
				$options .= $this->list_input( $args, $i );
				$i++;
	...
	public function oembed() {
		...
		echo esc_attr( $this->input )(
            array(
				'class'           => 'cmb2-oembed regular-text',
				'data-objectid'   => esc_attr( $this->field->object_id ),
				'data-objecttype' => esc_attr( $this->field->object_type ),
            )
        ),
		'<p class="cmb-spinner spinner" style="display:none;"></p>',
		'<div id="', esc_attr( $this->_id( '-status' ) ), '" class="cmb2-media-status ui-helper-clearfix embed_wrap">', esc_attr( $oembed ), '</div>';
	}
	...
	public function file_list() {
		...
		echo esc_attr( $this->input )(
            array(
				...
				'desc'             => '',
				'value'            => '',
				'data-previewsize' => esc_attr( is_array( $img_size ) ) ? sprintf( '[%s]', implode( ',', esc_attr( $img_size ) ) ) : 50,
            )
        ),
		esc_attr( $this->input )(
            array(
		...
		echo '<ul id="', esc_attr( $this->_id( '-status' ) ), '" class="cmb2-media-status cmb-attach-list">';
	...
	public function file() {
		...
		printf( '<input id="cmb2-image-upload-button" class="cmb2-upload-button button" type="button" value="%s" />', esc_html__($this->_text( 'add_upload_file_text', esc_html__( 'Add or Upload File', 'cmb2' ) )) );
		...
		'<div id="', esc_html( $this->_id( '-status' ) ), '" class="cmb2-media-status">';
		if ( ! empty( $meta_value ) ) {
	...
	public function img_status_output( $args ) {
		printf(
            '<%1$s class="img-status">%2$s<p class="cmb2-remove-wrapper"><a href="#" class="cmb2-remove-file-button"%3$s>%4$s</a></p>%5$s</%1$s>',
			$args['tag'],
			$args['image'],
			isset( $args['cached_id'] ) ? ' rel="' . esc_html( $args['cached_id'] ) . '"' : '',
			esc_html( $this->_text( 'remove_image_text', esc_html__( 'Remove Image', 'cmb2' ) ) ),
			wp_kses_post( isset( $args['id_input'] ) ) ? esc_html( $args['id_input'] ) : ''
		);
	}
	...
	public function file_status_output( $args ) {
		printf(
            '<%1$s class="file-status"><span>%2$s <strong>%3$s</strong></span>&nbsp;&nbsp; (<a href="%4$s" target="_blank" rel="external">%5$s</a> / <a href="#" class="cmb2-remove-file-button"%6$s>%7$s</a>)%8$s</%1$s>',
			$args['tag'],
