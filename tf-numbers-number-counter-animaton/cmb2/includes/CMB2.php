...
class CMB2 {
	...
	public function render_group( $args ) {
		...
		echo '<div class="cmb-row cmb-repeat-group-wrap"><div class="cmb-td"><div id="', esc_attr( $field_group->id() ), '_repeat" class="cmb-nested cmb-field-list cmb-repeatable-group', esc_attr( $sortable ), esc_attr( $repeat_class ), '" style="width:100%;">';

		if ( $desc || $label ) {
			$class = $desc ? ' cmb-group-description' : '';
			echo '<div class="cmb-row', esc_attr( $class ), '"><div class="cmb-th">';
			if ( $label ) {
				echo '<h2 class="cmb-group-name">', esc_attr( $label ), '</h2>';
			}
			if ( $desc ) {
				echo '<p class="cmb2-metabox-description">', esc_attr( $desc ), '</p>';
			}
			echo '</div></div>';
		}
		...
		if ( $field_group->args( 'repeatable' ) ) {
			echo '<div class="cmb-row"><div class="cmb-td"><p class="cmb-add-row"><button data-selector="', esc_attr( $field_group->id() ), '_repeat" data-grouptitle="', esc_attr( $field_group->options( 'group_title' ) ), '" class="cmb-add-group-row button">', esc_attr( $field_group->options( 'add_button' ) ), '</button></p></div></div>';
		}
	...
	public function render_group_row( $field_group, $remove_disabled ) {
		...
		echo '
		<div class="postbox cmb-row cmb-repeatable-grouping" data-iterator="', esc_attr( $field_group->index ), '">';

		if ( $field_group->args( 'repeatable' ) ) {
			echo '<button ', esc_attr( $remove_disabled ), 'data-selector="', esc_attr( $field_group->id() ), '_repeat" class="dashicons-before dashicons-no-alt cmb-remove-group-row"></button>';
		}
			...
			echo '
					<div class="cmb-row cmb-remove-field-row">
						<div class="cmb-remove-row">
							<button ',esc_attr( $remove_disabled ), 'data-selector="', esc_attr( $field_group->id() ), '_repeat" class="button cmb-remove-group-row alignright">', esc_attr( $field_group->options( 'remove_button' ) ), '</button>
						</div>
					</div>
					';
		}
	...
	public function object_id( $object_id = 0 ) {
		...
		switch ( $this->object_type() ) {
			case 'user':
				$object_id = $_REQUEST['user_id'] ? $action : $object_id;
				$object_id = ! $object_id && isset( $GLOBALS['user_ID'] ) ? $GLOBALS['user_ID'] : $object_id;
				break;

			default:
                                $object_id = isset( $GLOBALS['post']->ID ) ? isset( $GLOBALS['post']->ID ) : $object_id;
