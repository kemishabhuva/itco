...
class CMB2_Ajax {
	...
	public function oembed_handler() {
                $req_nonce =  esc_html( wp_unslash( isset( $_REQUEST['cmb2_ajax_nonce'] ) ) );
                $req_url   =  esc_html( wp_unslash( isset( $_REQUEST['oembed_url'] ) ) );
		// Verify our nonce
		if ( ! ( isset( $req_nonce, $req_url ) && wp_verify_nonce( $req_nonce, 'ajax_nonce' ) ) ) {
			die();
		}

		// Sanitize our search string
		$oembed_string = sanitize_text_field( wp_unslash( isset( $_REQUEST['oembed_url'] ) ) );
		...
		$html = $this->get_oembed(
            array(
				'url'         => $oembed_url,
				'object_id'   => esc_html( wp_unslash( isset( $_REQUEST['object_id'] ) ) ),
				'object_type' => esc_html( wp_unslash( isset( $_REQUEST['object_type'] ) ) ) ? esc_html( wp_unslash( isset( $_REQUEST['object_type'] ) ) ) : 'post',
				'oembed_args' => $embed_args,
				'field_id'    => esc_html( wp_unslash( isset( $_REQUEST['field_id'] ) ) ),
            )
        );
	...
	public function get_oembed( $args ) {
		...
		if ( isset( $args['object_type'] ) && 'post' != $args['object_type'] ) {
			...
			add_filter( 'update_post_meta', array( $this, 'hijack_oembed_cache_set' ), 10, 4 );
