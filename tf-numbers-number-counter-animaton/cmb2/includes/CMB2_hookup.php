...
class CMB2_hookup {
	...
	protected static function register_styles() {
		...
		wp_register_style( 'cmb2-styles', cmb2_utils()->url( "css/cmb2{$front}{$min}.css" ), $styles, '1.0' );
	...
	public function user_new_metabox( $section ) {
		...
		if ( $section == $this->cmb->prop( 'new_user_section' ) ) {
			...
			$this->cmb->object_id( esc_html( wp_unslash( isset( $_REQUEST['user_id'] ) ) )  ? esc_html( wp_unslash( isset( $_REQUEST['user_id'] ) ) ) : $object_id );
			$this->user_metabox();
		}
	}
	...
	public function save_post( $post_id, $post = false ) {
		...
		$post_type = $post ? $post->post_type : get_post_type( $post_id );
                $retrieved_nonce = filter_input( INPUT_POST, 'nonce_CMB2php_tf_stats_box' );
		$do_not_pass_go          = (
			! $this->cmb->prop( 'save_fields' )
			// check nonce
			|| ! isset( $retrieved_nonce )
			|| ! wp_verify_nonce( $retrieved_nonce, 'nonce_CMB2php_tf_stats_box' )
			// check if autosave
			...
			|| ! in_array( $post_type, $this->cmb->prop( 'object_types' ), true )
		);
		...
		// take a trip to reading railroad – if you pass go collect $200
		 ...
		 $this->cmb->save_fields( $post_id, 'post', $_POST );
                 ...
                 // added custom title when no title supplied. 
//                 if( empty( get_the_title($post_id) ))
//                 {      wp_update_post( array(
//                            'ID'         => $post_id,
//                            'post_title' => esc_html__( 'Number title goes here... - ', 'tf-numbers' ).$post_id,
//                        ));
//                 
//                 }
	...
	public function once( $action, $hook, $priority = 10, $accepted_args = 1 ) {
		$key = md5( maybe_serialize( func_get_args() ) );

		if ( in_array( $key, self::$hooks_completed, true ) ) {
			return;
		}
