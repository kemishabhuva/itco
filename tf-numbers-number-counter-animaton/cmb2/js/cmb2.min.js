...
jQuery(document).ready(function() {
  ...
  jQuery("#cmb2-image-upload-button").change(function() {
     
    if(this.val()) {
      jQuery("#_tf_bgct").prop("disabled", true);
      jQuery("#_tf_bgct").prop("checked", false);
    } else {
      jQuery("#_tf_bgct").prop("disabled", false);
    }
  });
  
  jQuery("#_tf_bgct").change(function() {
    if(this.checked) {
      jQuery("#cmb2-image-upload-button").prop("disabled", true);
     jQuery(".img-status").prop("disabled", true);;
      jQuery("#postbox-container-2 .wp-color-result:first").prop("disabled", true);
      jQuery("#cmb2-image-upload-button").prop("checked", false);
    } else {
      jQuery("#cmb2-image-upload-button").prop("disabled", false);
       jQuery(".img-status").prop("disabled", false);
      jQuery("#postbox-container-2 .wp-color-result:first").prop("disabled", false);
    }
  });
  var clickCount = 0;
  jQuery("#postbox-container-2 .wp-color-result:first").click(function() {
      clickCount++;
       if(clickCount == 1){
            //alert(jQuery("#postbox-container-2 .wp-color-result:first").html());
            jQuery("#cmb2-image-upload-button").prop("disabled", true);
            jQuery("#_tf_bgct").prop("disabled", true);
        }
        if(clickCount == 2){
